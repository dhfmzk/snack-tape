...
test('App Settings route wires export, import, and delete-all data actions', async () => {
  ...
  assert.match(dom.downloaded[0].download, /^snacktape-json-\d{8}-\d{6}-current-tape\.json$/);
  assert.match(dom.downloaded[1].download, /^snacktape-csv-\d{8}-\d{6}-current-tape\.csv$/);
...
test('App exports a JSON backup before deleting a mixtape when requested', async () => {
  ...
  assert.match(dom.downloaded[0].download, /^snacktape-json-\d{8}-\d{6}-delete-tape\.json$/);
