...
test('storage fallback names are language-neutral generated values', async () => {
  const { createDefaultSequence, normalizeSegment, normalizeSequence, normalizeStore } = await import('../src/shared/storage.js');
...
test('normalizeSequence filters out invalid segments', async () => {
  const { normalizeSequence } = await import('../src/shared/storage.js');
...
test('normalizeImportedStore returns null for non-object inputs', async () => {
  const { normalizeImportedStore } = await import('../src/shared/storage.js');
...
test('saveStore checks local storage quota before writing large stores', async () => {
  const { saveStore } = await import('../src/shared/storage.js');
