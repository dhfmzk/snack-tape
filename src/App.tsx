...
function screenFor(state: AppState, store: SnackTapeAppStore, i18n: I18n): HTMLElement {
  ...
  if (state.route === 'capture') {
    return Capture({
      ...
      onPreviewOut: () => void store.captureOutPreview(),
      onClearDraft: () => void store.clearDraft(),
      onNudgeDraft: (deltaSeconds, edge) => void store.nudgeDraft(deltaSeconds, edge),
      ...
      onDuplicateSegment: (segmentId) => void store.duplicateSelectedSegment(segmentId),
