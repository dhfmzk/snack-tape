...
import { createExportFilename, parseImportedStoreJson, serializeStoreCsv, serializeStoreJson, type ExportFormat } from '../shared/dataTransfer.js';
import {
  clearPlaybackState,
  clearSegmentDraft,
  createId,
...
export class SnackTapeAppStore {
  ...
  async exportData(format: ExportFormat): Promise<void> {
    ...
    this.downloadTextFile(createExportFilename(format, this.state.settings.language), mimeType, text);
